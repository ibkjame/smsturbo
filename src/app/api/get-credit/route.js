...
/**
 * API Route to fetch the credit balance from the external ISMS.ASIA API.
...
export async function GET(request) {
  ...
  try {
    const apiResponse = await fetch('https://portal.isms.asia/sms-api/profile/get', {
      ...
      // Use 'no-store' to ensure we always get the latest credit balance
      cache: 'no-store',
    });
    ...
    if (!apiResponse.ok) {
      ...
      return NextResponse.json({ 
        success: false, 
        message: `Failed to fetch credit from external API. Status: ${apiResponse.status}` 
    ...
    // 6. Validate the response structure and extract the credit balance
    if (data.status === 'success' && data.data?.account?.credit_balance !== undefined) {
      const creditBalance = data.data.account.credit_balance;
      // Return a successful response to our frontend with the credit balance
      return NextResponse.json({ success: true, credit: creditBalance });
    } else {
    ...
    console.error('Internal Server Error while fetching credit:', error);
    return NextResponse.json({ 
      success: false, 
